...
def pkcs7_unpad(data):
    ...
    if data[-pad_len:] != bytes([pad_len]) * pad_len:
        raise ValueError("PKCS#7 padding bytes are inconsistent.")
    return data[:-pad_len]

def coerce_key_input(value, label):
    n = normalize_hex(value)
    if not n:
        raise ValueError(f"{label} is required (up to 32 hex characters).")
    _validate_hex_chars(n, label)
    if len(n) > KEY_BLOCK_LEN:
        raise ValueError(f"{label} is too long ({len(n)} hex chars). Maximum is 32.")
    return n.ljust(KEY_BLOCK_LEN, "0")

def coerce_hex_input(value, label):
    """Single-block: accepts any length, uses first 16 bytes, pads if short."""
    n = normalize_hex(value)
    if not n:
...
def hex_to_padded_blocks(value, label):
    """Multi-block: split arbitrary hex into 16-byte blocks, PKCS7 last if needed."""
    n = normalize_hex(value)
    if not n:
        raise ValueError(f"{label} is required.")
    _validate_hex_chars(n, label)
...
def compare_hex_answer(answer, expected_hex, label, required=False):
    ...
    if not normalized:
        if required:
            return {"label": label, "answer": "", "expected": expected_hex, "status": "wrong", "message": "Missing answer"}
...
def build_coach_practice(mode, key, block, final_answer, round_answers):
    ...
    if mode not in ("encrypt", "decrypt"):
        mode = "encrypt"
    key_b   = parse_hex_key(key)
    block_b = parse_hex_block(block)
    rks     = key_expansion(key_b)
    if mode == "encrypt":
...
def key_expansion_word_details(key_b):
    ...
    for i in range(4, 44):
        ...
        step["word"] = f"{new_word:08x}"
        details.append(step)
    return details

# ─────────────────── WORD-DOC CRYPTO (fixed) ────────────────────────────────
def xor_blocks(left, right):
    return bytes(a ^ b for a, b in zip(left, right))

def aes_cbc_encrypt(plaintext_bytes, key_b):
    """AES-128-CBC encrypt. Returns iv (16 bytes) + ciphertext."""
    rks = key_expansion(key_b)
    iv  = secrets.token_bytes(16)
    prev = iv
    ct = bytearray()
    for i in range(0, len(plaintext_bytes), 16):
        blk = plaintext_bytes[i:i+16]
        enc = encrypt_block(xor_blocks(blk, prev), rks)
        ct.extend(enc); prev = enc
    return iv + bytes(ct)

def aes_cbc_decrypt(iv_plus_ct, key_b):
    """AES-128-CBC decrypt. First 16 bytes are IV."""
    iv = iv_plus_ct[:16]; ct = iv_plus_ct[16:]
    rks = key_expansion(key_b)
    prev = iv; pt = bytearray()
    for i in range(0, len(ct), 16):
        blk = ct[i:i+16]
        dec = decrypt_block(blk, rks)
        pt.extend(xor_blocks(dec, prev)); prev = blk
    return bytes(pt)
...
def build_doc_crypto_result(mode, key, storage):
    ...
    filename = secure_filename(storage.filename) or "document"
    data = storage.read()
    if not data:
    ...
    if mode == "encrypt":
        ...
        if not plaintext_str.strip():
            raise ValueError("The document is empty — nothing to encrypt.")
        padded     = pkcs7_pad(plaintext_str.encode("utf-8"))
        iv_ct      = aes_cbc_encrypt(padded, key_b)
        ct_hex     = iv_ct.hex()
        out_bytes  = make_docx_with_text(ct_hex)
        ...
        if not filename.lower().endswith(".docx"):
            raise ValueError("Upload the encrypted .docx produced by this tool.")
        if not DOCX_AVAILABLE:
            raise ValueError("python-docx not installed.")
        ct_hex_raw = extract_docx_text(data)
        ct_hex     = normalize_hex(ct_hex_raw)
        if not ct_hex or any(c not in "0123456789abcdef" for c in ct_hex):
            raise ValueError("File does not contain valid AES ciphertext. Upload the encrypted .docx.")
        if len(ct_hex) < 64 or len(ct_hex) % 2 != 0:
            raise ValueError("Ciphertext is too short or malformed.")
        iv_ct      = bytes.fromhex(ct_hex)
        padded_pt  = aes_cbc_decrypt(iv_ct, key_b)
        pt_bytes   = pkcs7_unpad(padded_pt)
        plaintext_str = pt_bytes.decode("utf-8", errors="replace")
...
def single_round():
    ...
    return render_template("single_round.html",
        key=key, block=block, round_idx=round_idx,
        steps_data=steps_data, error=error,
...
def word_doc():
    ...
    if request.method == "POST":
        try:
            ...
            error = str(e)
    return render_template("word_doc.html", key=key, mode=mode, result=result, error=error)
...
def avalanche():
    ...
    if request.method == "POST":
        try:
            if mode == "same_key":
                key1 = coerce_key_input(key1, "Key 1")
                block1 = coerce_hex_input(block1, "Block 1")
                block2 = coerce_hex_input(block2, "Block 2")
                key2 = key1; key2_b = parse_hex_key(key1); block2_b = parse_hex_block(block2)
            else:
            ...
            s1 = encrypt_block_with_rounds(block1_b, rks1)
            s2 = encrypt_block_with_rounds(block2_b, rks2)
...
def coach():
    error = None; practice_result = None
    ...
    return render_template("coach.html", error=error, practice_result=practice_result,
        ...
        final_answer=final_answer, round_answers=round_answers)
